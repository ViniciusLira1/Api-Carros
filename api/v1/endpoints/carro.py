...
from schemas.carros_schema import CarroSchema
...
async def post_carro(carro: CarroSchema, db: AsyncSession = Depends(get_session)):
    novo_carro = CarroModel(nome=carro.nome, marca=carro.marca, ano=carro.ano)
    db.add(novo_carro)
    await db.commit()
...
async def get_carro(carro_id:int, db:AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if carro:
            ...
            raise HTTPException(detail="Carro nao encontrado",status_code=status.HTTP_404_NOT_FOUND)
        

@router.put("/{carro_id}",response_model=CarroSchema,status_code=status.HTTP_202_ACCEPTED)
async def put_carro (carro_id,carro:CarroSchema, db:AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(CarroModel).filter(CarroModel.id == carro_id)
        result = await session.execute(query)
